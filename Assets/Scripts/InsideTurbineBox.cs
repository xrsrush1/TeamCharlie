...
public class InsideTurbineBox : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Turbine = GameObject.Find("windturbine");
        Debug.Log("windturbine found");
    ...
    public void InsideCollector(GameObject Obj)
    {
        ...
        //we has this script for 1 particular script, so we will compare with that set obj only, here with turbine

        if (GameObject.ReferenceEquals(Obj, Turbine))
        {
            ...
            TurbineCollected = true;
            Debug.Log(TurbineCollected);
        }
        else
